...
class FSDPCheckpointer(Checkpointer):
    def _is_fsdp_wrapped(self) -> bool:
        if is_fsdp(self.model):
            return True
        # fall back to scanning children; cached for repeated calls
        if not hasattr(self, "_has_fsdp_module"):
            self._has_fsdp_module = any(is_fsdp(m) for m in self.model.modules())
        return bool(self._has_fsdp_module)
    ...
    def save(self, name: str, **kwargs: Any) -> None:
        ...
        if self._is_fsdp_wrapped():
            # preserve FSDP sharding semantics only when the model is actually sharded
