...
class RandomResizeForegroundCrop(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        if fg_coords.numel() == 0:
            # fallback to center crop when no foreground is found
            fg_coords = torch.tensor([[h // 2, w // 2]], device=img.device)

        # randomly sample a scale
        scale = float(np.random.uniform(self.scale_lower, self.scale_upper))
        ...
        ratio = float(np.random.uniform(self.ratio_lower, self.ratio_upper))
        # multiply the width by the ratio but keep the height the same
        crop_w = max(1, min(int(crop_w * ratio), w))
        ...
        y_crop_min = int(np.clip(coord[0].item() - crop_h // 2, 0, h - crop_h))
        x_crop_min = int(np.clip(coord[1].item() - crop_w // 2, 0, w - crop_w))

        y_crop_max = y_crop_min + crop_h
        x_crop_max = x_crop_min + crop_w
