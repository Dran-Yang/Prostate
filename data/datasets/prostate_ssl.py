...
class ProstateSSL(MedicalVisionDataset):
    ...
    def _define_split_dir(self):
        # Override to avoid forcing train/val/test subfolders; we use CSV split.
    ...
    def _init_images(self):
        if self.split_csv:
            ...
            # Fallback: use all folders under root
            super()._init_images()
    ...
    def _check_size(self):
        # Skip strict size checking since split comes from CSV.
        logger.info(f"Found {len(self.images)} patients for split '{self.split}'.")

    def get_num_classes(self) -> int:
        # Self-supervised; no classification labels.
        return 0
    ...
    def get_target(self, index: int) -> torch.Tensor:
        # No classification target in SSL pretraining; return -1.
